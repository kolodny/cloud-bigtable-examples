...
Table.prototype.mutate = function(entries, callback) {
  ...
  var pendingKeys = transform(entries, (result, entry) => {
  ...
  var retryOpts = {
    retries: this.maxRetries_,
    shouldRetryFn: commonGrpc.Service.shouldRetryRequest_,
    objectMode: true,
    request: (_, onResponse) => {

      var entriesBatch = entries.filter(entry => pendingKeys[entry.key]);
      var reqOpts = {
        objectMode: true,
        tableName: this.id,
        entries: entriesBatch.map(Mutation.parse)
      };

      return this.requestStream(grpcOpts, reqOpts)
        .on('error', callback) // TODO(mkolodny): Is this correct?
        .on('data', function(obj) {
          obj.entries.forEach(function(entry) {
            var key = entriesBatch[entry.index].key;
            ...
            status.entry = entriesBatch[entry.index];
