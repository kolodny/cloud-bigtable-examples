...
Table.prototype.mutate = function(entries, callback) {
  ...
  var pendingKeys = new Set(entries.map(entry => entry.key));

  var mutationErrorsByKey = {};

  var onBatchResponse = () => {
    grpcOpts.stream = null;
    var maxRetries = this.maxRetries || 2;
    var currentAttempt = grpcOpts.retryOpts.currentAttempt;
    if (pendingKeys.size !== 0 && currentAttempt <= maxRetries) {
      console.log('trying again in ', retryRequest.getNextRetryDelay(currentAttempt))
      setTimeout(
        makeNextRequestBatch,
        retryRequest.getNextRetryDelay(currentAttempt)
      );
      return;
    }
    var err = null;
    var mutationErrors = Object.keys(mutationErrorsByKey)
      .filter(key => mutationErrorsByKey[key])
      .map(key => mutationErrorsByKey[key]);
  ...
  var makeNextRequestBatch = () => {
    var service = this.parent.parent.getService_({service: "Bigtable"});
    var oldMutateRows = service.mutateRows;
    if (!global.patched) {
      global.patched = true
      service.mutateRows = function() {

        var ret = through.obj();
        setTimeout(() => ret.emit('metadata'), 100);
        setTimeout(() => ret.emit('response', {code: 200, message: "OK"}), 200);
        setTimeout(() => ret.emit('data', {
          entries: [
            {index:0, status: {code: 0, message: "", details: []}},
            {index:1, status: {code: 0, message: "", details: []}},
            {index:2, status: {code: 0, message: "", details: []}},
          ]
        }), 300);
        setTimeout(() => ret.emit('end'), 400);



        // var ret = oldMutateRows.apply(this, arguments);
        // var oldEmit = ret.emit;
        // var oldOn = ret.on;
        // ret.emit = function(name) {
        //   if (['response', 'metadata', 'data', 'status', 'end'].indexOf(name) === -1) {
        //     return
        //   }
    ...
    var entryBatch = entries.filter(entry => pendingKeys.has(entry.key));
    reqOpts.entries = entryBatch.map(Mutation.parse);
    this.requestStream(grpcOpts, reqOpts)
      .on('data', function(obj) {
        obj.entries.forEach(function(entry) {
          var mutationEntry = entryBatch[entry.index]
          var key = mutationEntry.key;
