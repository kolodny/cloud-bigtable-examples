...
function retryRequest(requestOpts, opts, callback) {
  ...
  if (typeof opts.currentAttempt !== 'number') {
    opts.currentAttempt = DEFAULTS.currentAttempt;
  }
  ...
  function retryAfterDelay(currentAttempt) {
    if (streamMode) {
      resetStreams();
    }

    setTimeout(makeRequest, getNextRetryDelay(currentAttempt));
  }
  ...
  function onResponse(err, response, body) {
    ...
    if (opts.currentAttempt <= opts.retries && opts.shouldRetryFn(response)) {
      retryAfterDelay(opts.currentAttempt);
      return;
    }
